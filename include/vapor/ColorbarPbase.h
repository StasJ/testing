...
//! Create a ColorbarPbase object from scratch
 ...
 virtual ~ColorbarPbase();
	...
	string GetTitle() const {
		return GetValueString(_colorbarTitleTag, "");
	}

	//! Set the title text
	//! \param[in] text to display
	void SetTitle(string text) {
		SetValueString(_colorbarTitleTag, "Set colorbar title", text);
	}

	//! Determine if colorbar is enabled
	//! \return true if enabled
	bool IsEnabled() const {
		return (GetValueLong(_colorbarEnabledTag, (long) false) != 0);
	}

	//! Enable or disable colorbar
	//! \param[in] bool true if enabled
	void SetEnabled(bool val) {
		SetValueLong(_colorbarEnabledTag, "enable/disable colorbar", val);
	}
