...
public:
 ...
 public:
  ...
  float hAspectRatio() const {
	return(fabs ((_right-_left)/(_bottom-_top)));
  }
 ...
 bool Insert(const rectangle_t &rectangle, const S &payload) {
	...
	if (ar <= maxAspectRatio && ar >= (1.0/maxAspectRatio)) {
		...
		for (int i=0; i<n; i++) {
			...
			status &= node_t::insert(
				_nodes, _rootidx, 
				rectangle_t(splitLeft, rectangle._top, splitRight, rectangle._bottom),
				payload, _maxDepth
			);
		...
		for (int i=0; i<n; i++) {
			...
			status &= node_t::insert(
				_nodes, _rootidx, 
				rectangle_t(rectangle._left, splitTop, rectangle._right, splitTop),
				payload, _maxDepth
			);
 ...
 bool Insert(T left, T top, T right, T bottom, S payload) {
	return(Insert(rectangle_t(left, top, right, bottom), payload));
 }
...
private:
 ...
 public:
  ...
  static bool insert(
	std::vector <node_t > &nodes, size_t nidx, const rectangle_t &rec,
	S payload, size_t maxDepth
  ) { 
  ...
  static void get_payload_contains(
	...
	if (node._payloads.size()) {
		payloads.insert(payloads.end(), node._payloads.begin(), node._payloads.end());
	}
	if (node._is_leaf) return;

	for (int q=0; q<4; q++) {
		size_t child = node_t::quadrant(nodes, nidx, q);
		if (nodes[child]._rectangle.contains(x,y)) { 
			node_t::get_payload_contains(nodes, child, x,y,payloads);
		}
	}
  }
  ...
  static  void print(const std::vector <node_t > &nodes, size_t nidx, std::ostream &os) {
	...
	for (int i=0; i<node._level; i++) os << " ";
	os << "payload : ";
	for (int i=0; i<node._payloads.size(); i++) {
		os << node._payloads[i] << " ";
	}
	os << std::endl;
	if (! node._is_leaf) {
