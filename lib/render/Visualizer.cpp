	...
	double dirVec[3], double strHandleMid[3])
{
	...
	gluProject(
		strHandleMid[0],strHandleMid[1], strHandleMid[2],  mvMatrix, 
		pMatrix, viewport,&screenx,&screeny, &screenz
	);
	//Obtain the coords of a point in view:
	bool success = (0 != gluUnProject((GLdouble)winCoords[0],(GLdouble)winCoords[1],screenz, mvMatrix,
		pMatrix, viewport,pt, pt+1, pt+2));
	if (success){
