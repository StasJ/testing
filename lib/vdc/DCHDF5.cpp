...
std::vector <string> DCHDF5::getDimensionNames() const {
	map <string, DC::Dimension>::const_iterator itr;
...
vector <string> DCHDF5::getMeshNames() const {
...
bool DCHDF5::getCoordVarInfo(string varname, DC::CoordVar &cvar) const {
...
bool DCHDF5::getDataVarInfo( string varname, DC::DataVar &datavar) const {
...
bool DCHDF5::getBaseVarInfo(string varname, DC::BaseVar &var) const {
	map <string, DC::CoordVar>::const_iterator itr;
...
std::vector <string> DCHDF5::getDataVarNames() const {
	map <string, DC::DataVar>::const_iterator itr;
...
std::vector <string> DCHDF5::getCoordVarNames() const {
	map <string, DC::CoordVar>::const_iterator itr;
...
std::vector <string> DCHDF5::getAttNames(string varname) const {
	DC::BaseVar var;
	bool status = getBaseVarInfo(varname, var);
...
DC::XType DCHDF5::getAttType(string varname, string attname) const {
	DC::BaseVar var;
	bool status = getBaseVarInfo(varname, var);
...
int DCHDF5::closeVariable(int fd) {
...
template <class T>
int DCHDF5::_readRegionTemplate(
	int fd,
...
int DCHDF5::_InitHorizontalCoordinates(
	NetCDFCFCollection *ncdfc
) {
...
int DCHDF5::_get_vertical_coordvar(
...
//
//
int DCHDF5::_InitVerticalCoordinates(
	NetCDFCFCollection *ncdfc
) {
...
int DCHDF5::_InitTimeCoordinates(
	NetCDFCFCollection *ncdfc
) {
    //
    // Get names of data variables  in the CF data set that have 1, 2 or 3
...
//
int DCHDF5::_InitVars(NetCDFCFCollection *ncdfc) 
{
	_dataVarsMap.clear();
	_meshMap.clear();
