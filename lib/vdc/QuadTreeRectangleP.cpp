...
QuadTreeRectangleP::QuadTreeRectangleP(
	float left, float top, float right, float bottom, size_t max_depth,
	size_t reserve_size
...
QuadTreeRectangleP::QuadTreeRectangleP(const QuadTreeRectangleP &rhs) {
	...
	for (int i=0; i<_qtrs.size(); i++) {
		_qtrs[i] = new QuadTreeRectangle<float,pType>(*(rhs._qtrs[i]));
    }
}
...
bool QuadTreeRectangleP::Insert(
	std::vector <class QuadTreeRectangle<float,pType>::rectangle_t > rectangles,
...
bool QuadTreeRectangleP::Insert(
	const Grid *grid, size_t ncells
) {

	if (ncells == 0) {
		ncells = Wasp::VProduct(grid->GetCellDimensions());
	}
...
void QuadTreeRectangleP::GetPayloadContained(float x, float y, std::vector <Size_tArr3> &payloads) const {
	...
	for (auto itr = p.begin(); itr != p.end(); ++itr) {
		payloads.push_back(Size_tArr3 {(*itr)[0], (*itr)[1], 0});
	}
 }

void QuadTreeRectangleP::GetStats(
