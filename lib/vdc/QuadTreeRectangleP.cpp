...
QuadTreeRectangleP<T,S>::QuadTreeRectangleP(
	T left, T top, T right, T bottom, size_t max_depth,
	size_t reserve_size
...
template <typename T, typename S>
QuadTreeRectangleP<T,S>::QuadTreeRectangleP(size_t max_depth, size_t reserve_size) : _left(0.0), _right(1.0) {
...
template <typename T, typename S>
QuadTreeRectangleP<T,S>::QuadTreeRectangleP(const QuadTreeRectangleP &rhs) {
	_left = rhs._left;
	_right = rhs._right;

	_qtrs.resize(rhs._qtrs.size());
	for (int i=0; i<_qtrs.size(); i++) {
		_qtrs[i] = new QuadTreeRectangle<T,S>(*(rhs._qtrs[i]));
    }
}

template <typename T, typename S>
QuadTreeRectangleP<T,S> &QuadTreeRectangleP<T,S>::operator=(const QuadTreeRectangleP<T,S>& rhs) {
...
//	if (*this == rhs) return *this;
	...
	_qtrs.resize(rhs._qtrs.size());
	for (size_t i=0; i<rhs._qtrs.size(); i++) {
		_qtrs[i] = new QuadTreeRectangle<T,S>(*(rhs._qtrs[i]));
	}
	return *this;
 }
...
template <typename T, typename S>
bool QuadTreeRectangleP<T,S>::Insert(T left, T top, T right, T bottom, S payload) {
	...
	for (int i=0; i<_qtrs.size(); i++) {
		...
		if (left <= r && right > l) {
			status &= _qtrs[i]->Insert(left, top, right, bottom, payload);
		}
	}
	return(status);
 }
...
template <typename T, typename S>
bool QuadTreeRectangleP<T,S>::Insert(
	std::vector <class QuadTreeRectangle<T,S>::rectangle_t > rectangles,
...
cout << "INSERT 3\n";
	...
	for (int i=0; i<_qtrs.size(); i++) {
		...
		for (size_t j=0; j<parRectangles[i].size(); j++) {
			status &= _qtrs[i]->Insert(parRectangles[i][j], parPayloads[i][j]);
		}
	}
cout << "INSERT 4\n";
...
template <typename T, typename S>
void QuadTreeRectangleP<T,S>::GetPayloadContained(T x, T y, std::vector <S> &payloads) const {
    payloads.clear();
...
template <typename T, typename S>
void QuadTreeRectangleP<T,S>::GetStats(
