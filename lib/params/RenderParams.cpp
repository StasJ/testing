...
RenderParams::RenderParams(
	...
	if (node->HasChild(ColorbarPbase::GetClassType())) {
		_Colorbar = new ColorbarPbase(
			ssave,node->GetChild(ColorbarPbase::GetClassType())
		);
	}
	else {
...
RenderParams::RenderParams(const RenderParams &rhs
) : ParamsBase(rhs) {
...
int RenderParams::GetCompressionLevel() const {
	return GetValueLong(_CompressionLevelTag,0);
}
...
int RenderParams::GetRefinementLevel() const {
	return (GetValueLong(_RefinementLevelTag,0));
}
...
void RenderParams::SetHeightVariableName(string varname) {
	...
	SetValueString(
		_heightVariableNameTag,"Set height variable name",varname
	);
//	setAllBypass(false);
}
...
void RenderParams::SetColorMapVariableName(string varname) {
	...
	SetValueString(
		_colorMapVariableNameTag,"Set color map variable name",varname
	);
}
...
string RenderParams::_findVarStartingWithLetter(
...
RenderParams *RenParamsFactory::CreateInstance(
	string className, DataMgr *dataMgr, ParamsBase::StateSave *ssave, XmlNode *node
) {
...
RenParamsContainer::RenParamsContainer(
	...
	for (int i=0; i<node->GetNumChildren(); i++) {
		...
		RenderParams *rParams = RenParamsFactory::Instance()->CreateInstance(
			classname, dataMgr, ssave, eleNode
		);
		if (rParams == NULL) {
			SetDiagMsg(
				"RenParamsContainer::RenParamsContainer() unrecognized class: %s",
				classname.c_str()
			);
